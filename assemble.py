...
def assemble_site():
    ...
    try:
        ...
        print(f"Error assembling index.html: {e}")

def assemble_css():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    template_path = os.path.join(base_dir, 'assets', 'css', 'style.template.css')
    output_path = os.path.join(base_dir, 'assets', 'css', 'style.css')
    ...
    try:
        ...
        # Regex for CSS includes: /* include: path/to/file.css */
        pattern = r'/\*\s*include:\s*([\w/\\.-]+)\s*\*/'
        ...
        print(f"Error assembling style.css: {e}")
